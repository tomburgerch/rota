...
|---|---|
| `rota usage` | One row per seat, in four buckets: ACTIVE (metered), ALTERNATIVES (have capacity), UNMEASURED (quota unknown, not spent) and UNAVAILABLE (spent, or the seat has ended). Shows both windows, when each resets, how fresh the number is, and which seat live sessions are billing to right now. `rota usage --record <seat> <weekly-used-%>` stores a number read by hand for a seat the usage API will not answer for. |
| `rota switch` | Pick the best seat (soonest deadline with room left, where the deadline is `min(weekly reset, seat end)`) and make it active. `rota switch <seat>` names one. Idle sessions restart onto it; sessions mid-work are left alone and converge later. |
...
| `rota billing` | The usage table plus what each seat costs and when it renews, from `~/.config/claude-failover/billing.json` (no API exposes this, so you maintain it; start from `config/billing.example.json`). An unmeasured seat reads `????????? UNMEASURED` rather than a percentage, and the `--record` command that would answer it is printed under the table. |
...
**A cancelled seat is still a live seat until its end date, and saying otherwise wastes weeks you have already paid for.** A cancelled subscription keeps refreshing its weekly window until the day it ends, which is often two or three more full windows. rota used to render a cached weekly window that had rolled as `weekly window expired` under UNAVAILABLE; that string means "the number I have is stale, go and measure it" and it reads as "this account is finished". Measured 2026-08-21, two cancelled-but-live seats showed exactly that while each still had roughly two more weekly refreshes left, and every session wrote them off. Three facts are now separate: the seat has **ended** (past its `ends` date in `billing.json`) is UNAVAILABLE reading `seat ended <date>`, the quota is **spent** is UNAVAILABLE until the named reset, and the measurement is **unknown** is a fourth bucket, UNMEASURED, "quota UNKNOWN, not spent. Very possibly full; go and look". Ranking follows: the deadline is `min(weekly reset, seat end)`, because a cancelled seat is the *most* use-it-or-lose-it quota in the pool, with a fixed number of weekly windows left, ever. Both pickers call one function for that (`lib/rota-ranking.sh`) rather than each holding their own copy of the rule: for two days they did hold their own copies, and the interactive `rota switch` and the unattended keeper named different seats for exactly this shape, a cancelled seat ending before its next reset. Both tables say it: `rota billing` (also reached as `rota accounts`) renders an unmeasured seat as `????????? UNMEASURED` instead of the bare `-` that read like a spent `0%`, counts GONE IN to the NEXT reset rather than the one that has already rolled, marks a cancelled seat's final window `(LAST window)`, and never ranks a seat it has no number for. It needs `status` and `ends` in `billing.json`; without them every seat reads as active with no end date, exactly as before.

**The usage probe returns 429, and not only when many sessions share a token.** Fifteen panes on one seat will do it, but measured 2026-08-21 the endpoint returned 429 for two seats across six attempts over two and a half minutes with no live session on either and a valid credential in every pool dir, so "retry in a minute" is not always the answer and no retry loop rescues it. rota never presents a 429 as a number: the row falls back to the last good cached read, marked with its age, and stays marked stale until a live read succeeds, and nothing switches on a stale row. When there is no usable cached read either, the row goes to UNMEASURED rather than being reported as unavailable, because an unanswered probe says nothing about the seat. `rota usage --record <seat> <weekly-used-%>` is the way out: it stores the percentage the vendor's own usage page prints, stamped with the window it belongs to so it expires by itself, and it never displaces a live fetch or a cache still describing its own window.
...
lib/rota-ranking.sh     which seat is next; sourced by the engine AND the keeper
